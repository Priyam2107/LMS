...
function storeBook($conn, $param)
{
    ...
    if(empty($title))
    {
        ...
        $result = array("error"=>"Author name  is required");
        return $result;
    }
        ...
        $result = array("error"=>"Kindly select category if none matches then select other");
        return $result;
    }
...
//Function to update a book's data
...
function updateBook($conn, $param)
{
    ...
    $datetime = date("Y-m-d H:i:s");
    $sql = "update books set title = '$title', author = '$author', publication_year = '$publication_year' , isbn = '$isbn' , updated_at = '$datetime' where id = $id";
    $result['success'] =  $conn->query($sql);
    return $result;
}
